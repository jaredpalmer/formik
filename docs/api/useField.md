...
- `remove(shouldValidate?: boolean): void` - A function to remove field-related data (`value`, `errors`, `touched` etc) from the form. Calling this will trigger validation to run if `validateOnBlur` is set to `true` (which it is by default). You can also explicitly prevent/skip validation by passing a first argument as `false`.
