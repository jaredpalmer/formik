...
`useFormik` is a hook that helps you with building forms. It is a React Hook.
...
Since there is no provider component, you cannot rely on context and so this is incompatible with `useField` and `Field`. Instead, you should use `formik.values` (as above) or, for reusable components where you would normally use `useField` and `Field`, you should use `formik.getFieldProps`:
