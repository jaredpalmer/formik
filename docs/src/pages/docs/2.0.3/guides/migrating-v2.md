...
For most users, Formik v2 is backwards compatible with v1.x. However, there are a few breaking changes that came with adopting React Hooks. This guide/cheatsheet is a rundown of what's new in Formik v2.
...
- If you use TypeScript, Formik 2 uses the `unknown` type, so you must be on TypeScript 3.0 or higher
...
- `validateOnMount?: boolean = false` has been added
