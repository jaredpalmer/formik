...
export function useFormik<Values extends FormikValues = FormikValues>({
  ...
  const { validate, validationSchema } = props
  /**
   * Run validation against a Yup schema and optionally run a function if successful
   */
  const runValidateHandler = React.useCallback(
    (values: Values): FormikErrors<Values> => {
      return isFunction(validate) ? validate(values) : emptyErrors      
    },
    [validate]
  );
  ...
  const runFieldLevelValidations = React.useCallback(
    (values: Values): FormikErrors<Values> => {
      ...
      return fieldKeysWithValidation.length > 0
  ...
  const runAllValidations = React.useCallback(
    (values: Values) => {
        ...
        const combinedErrors = deepmerge.all<FormikErrors<Values>>(
          [runFieldLevelValidations(values), runValidationSchema(values), runValidateHandler(values)],
          { arrayMerge }
        );
    ...
    (values: Values = state.values) => {
      return unstable_runWithPriority(LowPriority, () => {       
          if (!!isMounted.current) {
            dispatch({ type: 'SET_ERRORS', payload: runAllValidations(values) });
          }               
      });
    }
  );
  ...
  const validateFormWithHighPriority = useEventCallback(
    (values: Values = state.values) => {
      const combinedErrors = runAllValidations(values)
  ...
  const submitForm = useEventCallback(() => {
    ...
    const combinedErrors = validateFormWithHighPriority();
