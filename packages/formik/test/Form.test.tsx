...
describe('<Form />', () => {
	it('should submit form 5 times while holding enter key pressed', async () => {
		...
		render(<Formik initialValues={{}} onSubmit={onSubmit}><Form name="Form"/></Formik>)
	...
	it('should submit form two times when pressing enter key, then unpressing it & repeat', async () => {
		const onSubmit = jest.fn();
		render(<Formik initialValues={{}} preventStickingSubmissions onSubmit={onSubmit}><Form name="Form"/></Formik>)
		const form = screen.getByRole('form');
