...
import {
  Formik as FormikSync,  
  FormikProps,
  FormikConfig,
} from '../src/sync';
import {
  Formik as FormikAsync,
...
function makeFormikRenderer(Formik: any) {
  return function renderFormik<V = Values>(props?: Partial<FormikConfig<V>>) {
  let injected: any;
...
[{
  name: 'Formik (async)',
  component: FormikAsync,
},
 {
   name: 'Formik (sync)',
   component: FormikSync
 }
].forEach(({ component: Formik, name }) => {
  const renderFormik = makeFormikRenderer(Formik)
  describe(name, () => {
