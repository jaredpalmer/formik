...
> {
  ...
  shouldComponentUpdate(
    ...
    if (typeof props.shouldFieldUpdateOrEnableDefault === 'function') {
      ...
      const changingFields = Object.keys(fieldChangingMap).filter(item => {
        return fieldChangingMap[item] && reg.test(item);
      });

      return (
        changingFields.length > 0 ||
        getIn(touched, name) !== getIn(props.formik.touched, name) ||
        getIn(errors, name) !== getIn(props.formik.errors, name)
      );
    }
    return true;
  }
