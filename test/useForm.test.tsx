...
import { useForm, FormikProps, FormikConfig } from '../src';
...
function renderFormik<V = Values>(props?: Partial<FormikConfig<V>>) {
  ...
  function FormikHook(p: FormikConfig<V>) {
    const [Form, formikProps] = useForm(p);
    ...
    const {
      ...
      errors,
      isSubmitting,
    } = (formikProps as unknown) as FormikProps<Values>;
...
describe('useForm()', () => {
  it('should initialize Formik state and pass down props', async () => {
    const { getProps } = await renderFormik();
  ...
  describe('handleChange', () => {
    ...
    it('updates values when passed a string (overloaded)', () => {
      ...
      function C() {
        const [Form, formikProps] = useForm({
  ...
  describe('handleReset', () => {
    ...
    // Was commented out when copied from Formik.test.tsx
  ...
  // Following were commented out when copied from Formik.test.tsx
  ...
  it('should warn against buttons with unspecified type', () => {
    function C() {
      const [Form, { handleChange, values }] = useForm({
        onSubmit: noop,
    ...
    const { getByText, getByTestId } = render(<C />);
