...
_NOTE_: In Formik&trade; v0.12 / 1.0, a new `meta` prop may be added to `Field` and `FieldArray` that will give you relevant metadata such as `error` & `touch`, which will save you from having to use Formik&trade;or lodash's getIn or checking if the path is defined on your own.
