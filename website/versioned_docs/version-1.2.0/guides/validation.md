...
Formik&trade; is designed to manage forms with complex validation with ease. Formik&trade;supports synchronous and asynchronous
...
much, Formik&trade; has a special config option / prop for Yup object schemas called `validationSchema` which will automatically transform Yup's validation errors into a pretty object whose keys match `values` and `touched`. This symmetry makes it easy to manage business logic around error messages.
...
You can manually trigger both form-level and field-level validation with Formik&trade; using the `validateForm` and `validateField` methods respectively.
...
You can control when Formik&trade;runs validation by changing the values of `<Formik validateOnChange>` and/or `<Formik&trade;validateOnBlur>` props depending on your needs. By default, Formik&trade;will run validation methods as follows:
...
No. Use `undefined` instead. Formik&trade; uses `undefined` to represent empty states. If you use `null`, several parts of Formik's computed props (e.g. `isValid` for example), will not work as expected.
