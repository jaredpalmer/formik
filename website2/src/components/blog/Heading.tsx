...
export const getTocDataFromHeading = (el: Element) => {
  return {
...
export default ({ children: component, id }: { children: any; id?: any }) => {
  ...
  return (
    <a
      href={`#${id}`}
      id={id}
      className={NOTION_ANCHOR_CLASSNAME}
      style={{ color: 'inherit' }}
      data-heading-text={collectText(text)}
      data-heading-depth={component.type.replace('h', '')}
    >
      {component}
    </a>
  );
